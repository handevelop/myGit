...
int token_parsing(int index) {
	...
	if (input_data[index][0] == '.') { //주석일때
		...
		if (tmp_token == NULL || tmp_token[0] == '\t' || tmp_token[0] == ' ') { //오퍼랜드가 없는 경우
			...
			parsing_cnt++;
			symbol_token[index + ltorg_line_sum].operand[0] = (char *)malloc(strlen(tmp) + 1);
			strcpy(symbol_token[index + ltorg_line_sum].operand[0], tmp); // 번째 오퍼랜드
...
void make_objectcode(char* file_name) {
	...
	int program_start_line = 0; // 프로그램의 시작할의 라인번호
	...
	int tmp_data = 0; // 주소(address)를 계산할 써주는 임시의 라인
	int current = 0; // 현재 위치
	FILE *fp = NULL;
	char *tmp;
	...
	while (is_line_end == 0) {
		...
		int tmp_end = 0;// 끝의 위치
		while (current <= program_end_line) { //끝날까지 텍스트 출력
			if (is_line_new == 1) {
